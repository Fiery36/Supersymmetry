import globals.Globals

import static gregtech.api.unification.material.Materials.*;
import gregtech.api.unification.material.MarkerMaterials;
import static gregtech.api.unification.ore.OrePrefix.dye;

REACTION_FURNACE = recipemap('reaction_furnace')
DISTILLERY = recipemap('distillery')
BR = recipemap('batch_reactor')
ION_EXCHANGE = recipemap('ion_exchange')

BR.recipeBuilder()
    .fluidInputs(fluid('ytterbium_concentrate') * 1000)
    .inputs(ore('dustSodiumHydroxide') * 9)
    .outputs(metaitem('dustYtterbiumHydroxide') * 7)
    .fluidOutputs(fluid('wastewater') * 1000)
    .duration(100)
    .EUt(Globals.voltAmps[2])
    .buildAndRegister()

BR.recipeBuilder()
    .inputs(ore('dustYtterbiumHydroxide') * 7)
    .fluidInputs(fluid('hydrochloric_acid') * 3000)
    .fluidOutputs(fluid('ytterbium_chloride_solution') * 6000)
    .duration(100)
    .EUt(120)
    .buildAndRegister()

DISTILLERY.recipeBuilder()
    .fluidInputs(fluid('ytterbium_chloride_solution') * 6000)
    .outputs(metaitem('dustYtterbiumChloride') * 4)
    .fluidOutputs(fluid('water') * 6000)
    .duration(200)
    .EUt(30)
    .buildAndRegister()

REACTION_FURNACE.recipeBuilder()
    .inputs(ore('dustYtterbiumChloride') * 4)
    .inputs(ore('dustLanthanum'))
    .outputs(metaitem('dustYtterbium'))
    .outputs(metaitem('dustLanthanumChloride') * 4)
    .duration(200)
    .EUt(480)
    .buildAndRegister()